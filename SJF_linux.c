...
void simulate_task(Process *p, int process_num) {
    ...
    switch(process_num) {
        ...
        case 1: // P2: Real-Time Price Calc
            p->interrupt_latency = LINUX_INTERRUPT_LATENCY * 0.75;
        ...
        case 4: // P5: Security Audit
            p->interrupt_latency = LINUX_INTERRUPT_LATENCY * 1.1;
            p->context_switch_time = LINUX_CONTEXT_SWITCH * 1.05;
            break;
        case 5: // P6: Background Logging
            p->interrupt_latency = LINUX_INTERRUPT_LATENCY * 1.2;
            p->context_switch_time = LINUX_CONTEXT_SWITCH * 1.1;
            break;
    }
    
    // Simulate OS-specific variations
    p->ipc_throughput = LINUX_IPC_THROUGHPUT * (0.9 + 0.15 * (rand() / (double)RAND_MAX));
...
int main() {
    ...
    MetricThreshold thresholds[] = {
        {"Interrupt Latency", 50.0, LINUX_INTERRUPT_LATENCY, "To 're-check' queue after short tasks finish"},
        {"Scheduling Jitter", 1.0, LINUX_SCHED_JITTER, "Ensures price calculations stay stable"},
        {"CPU Utilization", 70.0, 58.0, "SJF leaves more headroom for short bursts"},
        {"Context Switch Time", 5.0, LINUX_CONTEXT_SWITCH, "Impacts SJF's frequent queue re-evaluations"},
        {"IPC Throughput", 1.0, LINUX_IPC_THROUGHPUT, "Short jobs need fast data access"}
    };
    ...
    while (completed < NUM_PROCESSES) {
        ...
        if (next_index == -1) {
            ...
            usleep(1000);  // Wait 1ms in microseconds
            current_time++;
            continue;
        }
    ...
    printf("------------------------------------------------------------------\n");
    printf("                     PROJECT PULSE THRESHOLD ANALYSIS (Linux SJF)                            \n");
    printf("------------------------------------------------------------------\n");
    printf(" Metric                                     | Target  | Linux Value | Status     | Why It Matters for SJF             \n");
    printf("------------------------------------------------------------------\n");
    ...
    printf("------------------------------------------------------------------\n");
    printf("                     LINUX SJF OBSERVATIONS                         \n");
    printf("------------------------------------------------------------------\n");
    printf(" * Linux excels at SJF due to fast context switching (3.2us)        \n");
    printf(" * Low interrupt latency (28us) enables quick queue re-evaluation   \n");
    printf(" * Minimal jitter (0.8ms) ensures precise execution of short jobs   \n");
    printf(" * High IPC throughput (1.4GB/s) benefits data-intensive short tasks\n");
    printf(" * Perfect for real-time bidding where short jobs must be prioritized\n");
    printf("------------------------------------------------------------------\n\n");
    
    printf("------------------------------------------------------------------\n");
    printf("                    VALENTRA RECOMMENDATION                         \n");
    printf("------------------------------------------------------------------\n");
    printf(" Based on SJF performance analysis:                                 \n");
    printf("                                                                  \n");
    printf(" + Ubuntu 22.04 LTS is SUPERIOR for SJF scheduling                 \n");
    printf("   * 64%% lower context switch time (3.2us vs 8.5us)               \n");
    printf("   * 38%% lower interrupt latency (28us vs 45us)                   \n");
    printf("   * 56%% higher IPC throughput (1.4GB/s vs 0.9GB/s)               \n");
    printf("   * 33%% lower scheduling jitter (0.8ms vs 1.2ms)                 \n");
    printf("                                                                  \n");
    printf(" Recommendation: Use Linux SJF for Bidding Engine core logic       \n");
    printf("                 Use Windows for backend database operations       \n");
    printf("------------------------------------------------------------------\n");
